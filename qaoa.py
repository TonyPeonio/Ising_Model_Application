...
def evaluate_sample(division: Sequence[int], graph: rx.PyGraph) -> float:
    ...
    for u, v in graph.edge_list():
        if division[u] != division[v]:
            ...
            #sum the weights of existing cuts 
...
# In[5]:
    ...
    sample_circuit = optimized_circuit.copy()
    sample_circuit.measure_all()
    sampled_circuit = pm.run(sample_circuit)

    final_shots = 250_000
    job = backend.run(sampled_circuit, shots=final_shots)
