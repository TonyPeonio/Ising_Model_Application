...
def handle_counts(counts, final_shots, graph, circuit, objective_func_vals, optimized_circuit):
    final_distribution_bin = {
        key: val / final_shots for key, val in counts.items()
    }

    final_distribution_int = {
        int(key, 2): val / final_shots for key, val in counts.items()
    }
...
def main():
    ...
    #Start classical processing 
    final_shots = 250_000 #many many shots 
    print(f"final_shots: {final_shots}")
    job = backend.run(optimized_circuit, shots=final_shots) #just run it
    counts = job.result().get_counts() #get results 
    #run the qc many times 

    handle_counts(counts, final_shots, graph, optimized_circuit, objective_func_vals, optimized_circuit) #handle the results and print them out
